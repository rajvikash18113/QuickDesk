document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTicketDetail() {
        ...
        ticketDetailView.innerHTML = `<button id="back-to-dashboard">&larr; Back to Dashboard</button><div class="ticket-detail-header"><h2>${ticket.subject}</h2><p>${ticket.description}</p><div class="ticket-actions"><span class="status ${ticket.status.replace(' ', '-')}" style="background-color: #2563eb;border: 1px solid #000;    padding-top: 5px; padding-right: 20px; padding-bottom: 5px; padding-left: 20px;;border-radius: 4px;" >${ticket.status}</span>${canUpdateStatus ? `<select id="status-updater" data-id="${ticket.id}"><option value="Open" ${ticket.status === 'Open' ? 'selected' : ''}>Open</option><option value="In Progress" ${ticket.status === 'In Progress' ? 'selected' : ''}>In Progress</option><option value="Resolved" ${ticket.status === 'Resolved' ? 'selected' : ''}>Resolved</option></select>` : ''}<div class="vote-buttons" data-id="${ticket.id}"><button data-vote="up">👍</button><span>${ticket.votes || 0}</span><button data-vote="down">👎</button></div></div></div><div class="comments-section"><h3>Comments</h3><div id="comments-list">${commentsHtml}</div><form id="comment-form"><textarea id="comment-text" rows="3" placeholder="Add a comment..."></textarea><button type="submit">Post Comment</button></form></div>`;
    }
